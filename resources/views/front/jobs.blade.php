                            ...
                            @if($jobTypes->isNotEmpty())
                                @foreach($jobTypes as $jobType)
                                    <div class="form-check mb-2"> 
                                        <input class="form-check-input" name="job_type[]" type="checkbox" value="{{ $jobType->id }}" id="job-type-{{ $jobType->id }}" {{ in_array($jobType->id, Request::get('job_type', [])) ? 'checked' : '' }}>    
...
<script>
    $("#searchForm").submit(function(e){
        ...
        if (checkedJobTypes.length > 0) {
            url += '&job_type[]='+checkedJobTypes.join('&job_type[]='); 
        }
